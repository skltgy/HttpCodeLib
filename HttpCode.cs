 ...
 * 
 * 解决 Deflate压缩后的编码识别乱码的问题
 * 移除ClearCookie 中Again标签的失误
 * 重写HttpItems.Header属性(如果填写HeaderStr自动返回格式化后的头数据)
 * 增加HttpItems.UserAgentTypeEx属性.(发送时UserAgent携带的内容属性.默认为PC 数据可选类型:PC/手机(安卓5.1.1)/WeChat)
 * 优化核心资源占用,暂无做大批量数据测试, testing
 * 删除从流中识别编码的方法,还原从头中识别
 * 不再校验Post字节数为空时(如果为空 request.ContentLength=0)
 * 新增 ProxyType[LocalProxy|NULL] 默认为NULL(不使用代理),如果需要使用本地代理设置为LocalProxy即可.填写ProxyIP则使用填写值.
*/
using System;
using System.Text;
using System.Net;
    ...
    public class HttpHelpers
    {
        ...
        private HttpResults GetResponesInfo(ref HttpItems objHttpItems)
        {
            ...
            using (_stream = new MemoryStream())
            {
                ...
                {
                    //处理 Deflate
                    _stream = GetMemoryStream(new DeflateStream(response.GetResponseStream(), CompressionMode.Decompress));
                }
                else
                {
            ...
            {
                result.Html = "HttpCode接收数据出现异常..请检查数据包.当前状态码:" + result.StatusCodeNum.ToString() + result.Html + "异常信息: " + exp.ToString();
            }
        ...
        private void SetRequest(HttpItems objHttpItems)
        {
            ...
            if (objHttpItems.Connectionlimit > 0)
            {
                ...
                System.Net.ServicePointManager.DefaultConnectionLimit = objHttpItems.Connectionlimit;
                System.Net.ServicePointManager.SetTcpKeepAlive(true, 5000, 2000);//使用TCP的检测方式  testing
